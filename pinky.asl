...
+active : pacman_at(PacX,PacY) & prev_pacman_at(PrevPacX,PrevPacY) & mode(M) <-
    .print("Pinky is alive");
    navigation.decide_target(2, PacX, PacY, PrevPacX, PrevPacY, M, TX, TY);
    +target(TX,TY);
    //.print("Targeting ", TX, ",", TY); 
    !approach_target.

// pinky at crossroad decides where to go depending on pacman position
+!approach_target : at(X,Y) & prev_at(PrevX,PrevY) & crossroad(X,Y) & pacman_pos(PacX,PacY) & prev_pacman_pos(PrevPacX,PrevPacY) & mode(M) <-
    ...
    !approach_target.
// pinky in corridor advances coherently with the past movement
...
// pinky or another ghost killed the pacman, stops execution
+stop : true <-
...
// the pacman collected all the dots, pinky surrenders
